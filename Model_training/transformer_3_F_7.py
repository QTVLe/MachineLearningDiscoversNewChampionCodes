...
def predict(model, code):
    code = code.unsqueeze(0).to(device, dtype=global_dtype)
    length = torch.tensor([code.shape[1]]).to(device)

    prob, _ = model(code, length)
    vector = torch.unsqueeze(torch.arange(36, dtype=global_dtype), 0) # shape (1,36)
    expectation = torch.sum(prob * vector, dim=1)

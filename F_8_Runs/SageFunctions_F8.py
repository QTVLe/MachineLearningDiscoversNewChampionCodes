...
class SageFunctions():
    def __init__(self, prime):
        ...
        self.symbol_mapping_magma = {'0':0, '1':1, '$.1':2, '$.1^2':3, '$.1^3':4, '$.1^4':5, '$.1^5':6, '$.1^6':7}
        self.symbol_mapping_sage = {
            '0': 0,
            '1': 1,
            'z3': 2,
            'z3^2': 3,
    ...
    def vertices2GeneratorObj(self, vertices):
        toric_matrix = self.vertices2toricMatrix_(vertices)
        basis = toric_matrix.row_space().basis()
        generator = matrix(self.F, basis)
        return generator
    
    def vertices2GeneratorMatrix(self, vertices):
        generator = self.vertices2GeneratorObj(vertices)
        generator_strings = []
        for row in generator:
            generator_strings.append([self.symbol_mapping_sage[str(x)] for x in row])
        return np.array(generator_strings, dtype=global_np_dtype)
